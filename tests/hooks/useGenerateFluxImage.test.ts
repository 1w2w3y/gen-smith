...
describe("useGenerateFluxImage", () => {
  ...
  it("returns initial state", () => {
    const { result } = renderHook(() => useGenerateFluxImage());
  ...
  it("sets images on successful generation with format png", async () => {
    vi.mocked(fetch).mockResolvedValue(
      new Response(
        JSON.stringify({
          images: [{ b64_json: "fluximage", index: 0 }],
        }),
        { status: 200 }
      )
    );
    ...
    expect(result.current.images![0].b64_json).toBe("fluximage");
    expect(result.current.images![0].format).toBe("png");
    expect(result.current.error).toBeNull();
  });

  it("calls /api/image/flux/generate endpoint", async () => {
    vi.mocked(fetch).mockResolvedValue(
      new Response(
  ...
  it("sets error on API failure", async () => {
    vi.mocked(fetch).mockResolvedValue(
      new Response(
        JSON.stringify({
          error: { code: "api_error", message: "FLUX API failed" },
        }),
        { status: 500 }
      )
    );
    ...
    expect(result.current.images).toBeNull();
    expect(result.current.error).toBe("FLUX API failed");
  });
